...
export async function getServerSideProps(context) {
  ...
  try {
    ...
    console.error("Error fetching estore:", error);
  }
...
export default function EstorePage({ estoreData }) {
  useEffect(() => {
    if (!estoreData?.id) return;
    ...
    const appLink = `hillsgo://estore/${estoreData.id}`;
    const fallbackLink = `https://www.hillsgo.com/estore/${estoreData.id}`;
    const now = Date.now();
        ...
        <div>
          <div className="table-container">
            <div style={styles.titleBox}>
              <p className="subTitle" style={styles.estoreTitle}>
                Estore Details
              </p>

              <img
                src={estoreData?.imageUrl} // Assuming first image is used for the card
                alt="image detail"
                style={styles.image}
              />
            </div>
            ...
            <table>
              <tbody>
                <tr>
                  <th>Estore Name: </th>
                  <td>{estoreData?.estoreName}</td>
                </tr>

                <tr>
                  <th>Description: </th>
                  <td>{estoreData?.estoreDescription}</td>
                </tr>
                <tr>
                  <th>State: </th>
                  <td>{estoreData?.estoreState}</td>
                </tr>
              </tbody>
            </table>
          </div>
          <div style={styles.card}>
            ...
            <p style={styles.infoText}>
              The products for the estore is currently viewable only on our
