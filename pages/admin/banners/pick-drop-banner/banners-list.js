...
const ListBanners = () => {
  ...
  useEffect(() => {
    const fetchBanners = async () => {
      try {
        const querySnapshot = await getDocs(collection(db, "pickDropBanners"));
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this banner?")) {
      try {
        await deleteDoc(doc(db, "pickDropBanners", id));
    ...
    <Admin>
      <AdminLayout>
        ...
        <div style={styles.container}>
          <h2>Pick & Drop Banners</h2>
          {banners.length === 0 ? (
            <p>No Pick & Drop banners found.</p>
          ) : (
                ...
                <div key={banner.id} style={styles.bannerCard}>
                  ...
                  <div style={styles.buttonContainer}>
                    <button
                      ...
                      onClick={() =>
                        router.push(
                          `/admin/banners/pick-drop-banner/${banner.id}`
                        )
                      }
                    >
                      Edit
