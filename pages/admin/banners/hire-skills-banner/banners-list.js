...
const ListBanners = () => {
    ...
    const fetchBanners = async () => {
      try {
        const querySnapshot = await getDocs(
          collection(db, "hireSkillsBanners")
        );
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this banner?")) {
      try {
        await deleteDoc(doc(db, "hireSkillsBanners", id));
    ...
    <Admin>
      <AdminLayout>
        ...
        <div style={styles.container}>
          <h2>Hire Skills Banners</h2>
          {banners.length === 0 ? (
            <p>No Hire Skills banners found.</p>
          ) : (
                ...
                <div key={banner.id} style={styles.bannerCard}>
                  ...
                  <div style={styles.buttonContainer}>
                    <button
                      ...
                      onClick={() =>
                        router.push(
                          `/admin/banners/hire-skills-banner/${banner.id}`
                        )
                      }
                    >
                      Edit
